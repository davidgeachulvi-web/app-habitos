/* AWAKE — hábitos, agenda y ficha. Script global; se carga antes de app.js. */
        ...
        // Mensajes de confirmación al quitar un sello: indican con claridad el
        // tipo de elemento (tarea/hábito, actividad, abstinencia o deseo) y lo
        // que implica quitarlo. Textos revisados (B-60).
        function textoConfirmacionQuitarSello(opts) {
            opts = opts || {};
            const tipo = opts.tipo || 'tarea';
            const nombre = String(opts.nombre || '').trim();
            const ref = nombre ? '«' + nombre + '»' : (tipo === 'deseo' ? 'este deseo' : 'esta tarea');
            const adj = opts.adjuntos ? ' (incluida su foto o nota)' : '';
            if (tipo === 'deseo') {
                return {
                    titulo: 'Quitar el sello del deseo',
                    texto: 'Vas a quitar el sello del deseo ' + ref + '. Volverá a quedar pendiente y su registro saldrá del historial' + adj + '.'
                };
            }
            if (tipo === 'abstinencia') {
                const sello = opts.sellado ? ' Se quitará el sello del día.' : '';
                return {
                    titulo: 'Marcar recaída',
                    texto: 'Vas a marcar una recaída en ' + ref + '.' + sello + ' La racha se romperá.'
                };
            }
            let fragMomento = '';
            if (opts.momento) {
                let m;
                try { m = etiquetaMomentoHabito(opts.momento); } catch (e) { m = ''; }
                if (m && m !== 'Cualquier momento') fragMomento = ' · ' + m;
            }
            const intro = tipo === 'actividad' ? 'de la actividad ' : 'de la tarea ';
            return {
                titulo: 'Quitar el sello',
                texto: 'Vas a quitar el sello ' + intro + ref + fragMomento + '. Se eliminará el registro de ese día' + adj + '.'
            };
        }
...
try { if (window.awakeAnalytics) window.awakeAnalytics.track('habit_created', { nombre: nuevoHabito.nombre || '', tipo: nuevoHabito.tipo || '' }); } catch (e) {}
                ...
                if (verCumplidos) {
                    ...
                    div.innerHTML = `
                        <div class="habit-card-left">
                            <div class="task-checkbox checked" onclick="solicitarDesmarcarDeseo(${originalIdx})" title="Marcar como pendiente">✓</div>
        ...
        function clicCheckboxDeseo(index) {
            ...
            if (window.pintarEtiquetaAdjunto) pintarEtiquetaAdjunto(document.getElementById('wish-completion-image'));
...
try { if (window.awakeAnalytics) window.awakeAnalytics.track('deseo_completed', { nombre: cleanHabitName(d.nombre) }); } catch (e) {}
        ...
        function solicitarDesmarcarDeseo(index) {
            const d = misDeseos[index];
            if (!d || !d.completado) return;
            resetConfirmModalButtons();
            const aviso = textoConfirmacionQuitarSello({
                tipo: 'deseo',
                nombre: cleanHabitName(d.nombre),
                adjuntos: !!(d.imagen || (d.comentario && d.comentario.trim()))
            });
            document.getElementById('confirm-modal-title').textContent = aviso.titulo;
            document.getElementById('confirm-modal-text').textContent = aviso.texto;
            confirmModalAction = async () => desmarcarDeseo(index);
            document.getElementById('confirm-modal').classList.add('active');
        }
        ...
        async function desmarcarDeseo(index) {
            ...
            guardarEstadoLocal();
            avisarDiarioRemoto();
            if (typeof revocarInsigniasSinMetrica === 'function') revocarInsigniasSinMetrica();
            renderizarListaDeseos();
            renderizarPerfilPublicacionesGrid();
            renderizarTabHistorial();
            actualizarEstadisticasPerfil();
        ...
        async function insertarRegistroMarca({ index, momento, dateTarget, texto, toast, celebrar, privacy }) {
            ...
            // Si este sello terminará celebrando (sello.mp3/logro), el sellado puede
            // tardar (red, foto): cancelar YA el click genérico pendiente del gesto
            // para que nunca suene click.mp3 a la vez que el sonido de logro.
            if (celebrar && typeof marcarSonidoEspecifico === 'function') {
...
try { if (typeof cancelarAvisoRiesgoHoySiProcede === 'function') cancelarAvisoRiesgoHoySiProcede(); } catch (e) {}
        ...
        async function desellarSellosHabitoEnFecha(habitRef, dateTarget, momento) {
            ...
            try {
                ...
                // Sonido de eliminación al QUITAR el sello: se reproduce cuando el
                // sello se elimina de verdad (tras confirmar el aviso), nunca al
                // pulsar el checkbox. reproducirSonidoEliminar cancela el click
                // genérico pendiente para que sea el único sonido del gesto.
                if (typeof reproducirSonidoEliminar === 'function') {
                    try { reproducirSonidoEliminar(); } catch (e) {}
                }
        ...
        function quizasCelebrarDiaSellado(items) {
            ...
            try {
                if (localStorage.getItem('proto2awake_day_sealed_' + key) === '1') {
                    daySealedToastKey = key;
                    return;
                }
                localStorage.setItem('proto2awake_day_sealed_' + key, '1');
            } catch (e) {}
            daySealedToastKey = key;
        ...
        function mostrarHojaDiaSellado(rachaDias) {
            if (typeof marcarSonidoEspecifico === 'function') marcarSonidoEspecifico();
        ...
        function abrirModalFotoNota(index, momento) {
            ...
            if (imgEl && window.pintarEtiquetaAdjunto) pintarEtiquetaAdjunto(imgEl);
        ...
        function ejecutarLogicaCheckboxHabito(index, momento, dateTarget, completadoHoy) {
            ...
            if (habitEsAbstinencia(h)) {
                ...
                resetConfirmModalButtons();
                const avisoRecaida = textoConfirmacionQuitarSello({
                    tipo: 'abstinencia',
                    nombre: cleanHabitName(h.nombre),
                    sellado: !!completadoHoy
                });
                document.getElementById('confirm-modal-title').textContent = avisoRecaida.titulo;
                document.getElementById('confirm-modal-text').textContent = avisoRecaida.texto;
                confirmModalAction = async () => {
            ...
            if (momento === '24/7') {
                if (completadoHoy) {
                    // Aviso de confirmación al quitar el sello de un hábito
                    // «Todo el día / 24/7» (antes se desellaba al instante, sin
                    // preguntar). Igual que el desellado normal.
                    const aviso = textoConfirmacionQuitarSello({
                        tipo: 'tarea',
                        nombre: cleanHabitName(h.nombre),
                        momento: '24/7',
                        adjuntos: (function () {
                            try {
                                return logsADesellarHabito(h, dateTarget, '24/7').some(l => logHabitoTieneFoto(l) || notaRealDeSello(l));
                    ...
                    document.getElementById('confirm-modal-text').textContent = aviso.texto;
                    confirmModalAction = async () => {
                        await desellarSellosHabitoEnFecha(h, dateTarget, '24/7');
                        if (!h.enDescanso) {
            ...
            if (completadoHoy) {
                const aviso = textoConfirmacionQuitarSello({
                    tipo: naturalezaHabito(h) === 'unaVez' ? 'actividad' : 'tarea',
                    nombre: cleanHabitName(h.nombre),
                    momento,
                    adjuntos: (function () {
                        try {
                            return logsADesellarHabito(h, dateTarget, momento).some(l => logHabitoTieneFoto(l) || notaRealDeSello(l));
                        } catch (e) { return false; }
                    })()
                });
                document.getElementById('confirm-modal-title').textContent = aviso.titulo;
        ...
        async function guardarCompletado() {
            ...
            if (existing) {
                const aplicarUpdate = async (imagenUrl) => {
                    if (imagenUrl && esDataUrlMedia(imagenUrl) && currentUser) imagenUrl = null;
            ...
            // Sellado NUEVO: puede tardar (foto, red, supabase). Cancelar YA el
            // click genérico pendiente del gesto de confirmación para que el
            // único sonido sea el logro del sello (nunca click.mp3 + sello.mp3).
            ...
            const procesarRegistro = async (imagenUrl) => {
                selloOperacionEnCurso();
                // Sellado con foto/nota: puede tardar (compresión + subida). Cancelar
                // el click genérico pendiente: solo sonará el logro del sello.
                if (typeof marcarSonidoEspecifico === 'function') {
                    try { marcarSonidoEspecifico(); } catch (e) {}
                }
                try {
