/* AWAKE — mensajes y chat. Script global; se carga antes de app.js. */
        ...
        function anclarGrabacionAudioChat() {
            ...
            pulsoHaptico(16);
            reproducirSonido('chat_rec_bloqueo');
        ...
        async function iniciarGrabacionAudioChat() {
            ...
            if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia || typeof MediaRecorder === 'undefined') {
                mostrarToastLujo('Este dispositivo no puede grabar audio', { tipo: 'error' });
            ...
            try {
                ...
                chatMediaRecorder.onerror = () => {
                    mostrarToastLujo('Se interrumpió la grabación', { tipo: 'error' });
                    chatRecordCancel = true;
                ...
                pulsoHaptico(18);
                reproducirSonido('chat_rec_inicio');
                ...
                mostrarToastLujo(denied
                    ...
                    : 'No se pudo iniciar la grabación', { tipo: 'error' });
            }
        }
        ...
        function detenerGrabacionAudioChat(cancelar) {
            ...
            if (cancelar) {
                pulsoHaptico(26);
                reproducirSonido('chat_rec_descarte');
            } else {
                pulsoHaptico(14);
                reproducirSonido('chat_rec_envio');
            }
        ...
        async function enviarMensajeDirecto(extra = {}) {
            ...
            if (image && (!imageUrl || esDataUrlMedia(imageUrl))) {
                mostrarToastLujo('No se pudo enviar la imagen.', { tipo: 'error' });
                return;
            }
            if (audio && (!audioUrl || esDataUrlMedia(audioUrl))) {
                mostrarToastLujo('No se pudo enviar el audio.', { tipo: 'error' });
                return;
            }
            const payload = {
                text,
