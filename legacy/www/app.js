        ...
        let savedBg = parseInt(localStorage.getItem('monolith_bg_choice'));
        ...
        let activeBackground = localStorage.getItem('monolith_bg_choice') == null ? 2 : normalizarFondo(savedBg);
        ...
        const FEEDBACK_PREFS_KEY = 'awake_feedback_prefs';
        const IOS_ADAPT_KEY = 'awake_ios_adapt';
        let prefsFeedback = { sonido: true, vibracion: true };
        ...
        const CHAT_READS_KEY = 'awake_chat_last_read';
        let lecturasChatHidratadasPara = null;
        const ONBOARD_KEY = 'awake_onboarded';
        const FIRST_SEAL_KEY = 'awake_first_seal';
        ...
        async function insertarHabitoNubeConReintento(nuevoHabito) {
            ...
            for (let i = 0; i < 3; i++) {
                ...
                if (!error || !errorEsRecursoAusente(error) || !suavizarColumnasHabitoAusentes()) {
                    mostrarToastLujo('No se pudo guardar el hábito en la nube. Revisa la conexión.');
                    return null;
                }
            }
            return null;
        ...
        async function subirImagenOAvisar(dataUrl, folder, extHint) {
            ...
            mostrarToastLujo(currentUser ? 'No se pudo guardar la foto. Inténtalo de nuevo.' : 'Inicia sesión para guardar la foto.');
            return '';
        }
...
try { if (window.awakeAnalytics) window.awakeAnalytics.track('onboarding_done'); } catch (e) {}
            ...
            sincronizarCapaPrimerRitual();
            if (conHabito) {
                mostrarToastLujo('Toca el recuadro para sellar el hábito');
                quizasMostrarGuiaPermisosRitual();
            } else mostrarToastLujo('Añade tu primer hábito cuando quieras');
        }
        ...
        async function deshacerUltimoSellado() {
            ...
            badgeToastPendiente = null;
            if (!snap) {
                mostrarToastLujo('Nada que deshacer');
                return;
            }
            const h = misHabitos[snap.index];
            if (!h) {
                insigniasPendientesSello = [];
                mostrarToastLujo('No se pudo deshacer');
                return;
            }
            ...
            try {
                ...
                mostrarToastLujo('Sello deshecho');
            } finally {
...
try { if (window.awakeAnalytics) window.awakeAnalytics.track('auth_completed'); } catch (e) {}
        ...
        function seleccionarFondoApp(id) {
            ...
            localStorage.setItem('monolith_bg_choice', id);
            initActiveBackgroundEngine();
        ...
        function playCosmicWhisper() {
            if (!prefsFeedback.sonido) return;
            try {
                const audioCtx = obtenerAudioCosmico();
                if (!audioCtx) return;
                tonoSuaveAwake(audioCtx, 196, audioCtx.currentTime, 0.28, 0.07);
            } catch (err) {}
        }

        function tonoSuaveAwake(ctx, freq, start, dur, peak) {
            ...
            sub.frequency.setValueAtTime(Math.max(90, freq * 0.5), start);
            harm.frequency.setValueAtTime(freq * 2, start);
            const p = peak == null ? 0.13 : peak;
            gain.gain.setValueAtTime(0.0001, start);
            gain.gain.exponentialRampToValueAtTime(p, start + 0.012);
            gain.gain.exponentialRampToValueAtTime(0.0001, start + dur);
            gainSub.gain.setValueAtTime(0.0001, start);
            gainSub.gain.exponentialRampToValueAtTime(p * 0.32, start + 0.016);
            gainSub.gain.exponentialRampToValueAtTime(0.0001, start + dur + 0.03);
            gainHarm.gain.setValueAtTime(0.0001, start);
            gainHarm.gain.exponentialRampToValueAtTime(p * 0.12, start + 0.01);
            gainHarm.gain.exponentialRampToValueAtTime(0.0001, start + dur * 0.7);
            osc.connect(gain);
            sub.connect(gainSub);
            harm.connect(gainHarm);
            ...
            sub.stop(start + dur + 0.06);
            harm.stop(start + dur + 0.03);
        }

        function sonidoChatGrabacion(tipo) {
            if (!prefsFeedback.sonido) return;
            try {
                ...
                if (tipo === 'start') {
                    tonoSuaveAwake(ctx, 494, now, 0.12, 0.11);
                } else if (tipo === 'lock') {
                    tonoSuaveAwake(ctx, 392, now, 0.1, 0.1);
                    tonoSuaveAwake(ctx, 523, now + 0.08, 0.13, 0.09);
                } else if (tipo === 'discard') {
                    tonoSuaveAwake(ctx, 262, now, 0.16, 0.1);
                } else if (tipo === 'send') {
                    tonoSuaveAwake(ctx, 440, now, 0.1, 0.11);
                    tonoSuaveAwake(ctx, 554, now + 0.08, 0.14, 0.09);
                }
            } catch (e) {}
        }
        ...
        function feedbackToqueUi() {
            ...
            try {
                ...
                tonoSuaveAwake(ctx, 528, t, 0.11, 0.13);
                tonoSuaveAwake(ctx, 792, t + 0.045, 0.13, 0.08);
            } catch (e) {}
        }

        function sonidoSello() {
            if (!prefsFeedback.sonido) return;
            const now = Date.now();
            if (now - ultimoToqueUiMs < 90) return;
            ultimoToqueUiMs = now;
            try {
                ...
                if (!ctx) return;
                const t = ctx.currentTime;
                tonoSuaveAwake(ctx, 528, t, 0.14, 0.14);
                tonoSuaveAwake(ctx, 396, t + 0.05, 0.16, 0.09);
            } catch (e) {}
        }
        ...
        window.addEventListener('pointerup', (e) => {
            ...
            if (esObjetivoToqueUi(e.target)) feedbackToqueUi();
        }, true);
        window.addEventListener('click', (e) => {
            empujarEstrellasEnToque(e.clientX, e.clientY, clickEsSobreFondo(e));
            if (esObjetivoToqueUi(e.target)) {
                feedbackToqueUi();
                return;
            }
            if (clickEsSobreFondo(e) && !respetaMenosMovimiento()) playCosmicWhisper();
        }, true);
        ...
        const AWAKE_STATE_KEY = 'monolith_app_state';
        ...
        function aplicarPreferenciasVisualesLocal() {
            ...
            if (localStorage.getItem('monolith_bg_choice') != null) {
                activeBackground = normalizarFondo(localStorage.getItem('monolith_bg_choice'));
        ...
        function serializarHistorial(hist) {
            ...
            Object.keys(hist).forEach(key => {
                ...
                const mapped = hist[key].map(item => {
                    const image_url = esDataUrlMedia(item.image_url) ? '' : (item.image_url || '');
                    const imagenes = (item.imagenes || []).filter(u => u && !esDataUrlMedia(u));
                    return {
                        ...item,
        ...
        async function solicitarPermisoNotificacionesWeb() {
            if (!notificacionesWebDisponibles()) return Notification && Notification.permission;
        ...
        function renderizarFeedActividad(container) {
            ...
            Object.values(historialAgrupado).forEach(ejecuciones => {
                ejecuciones.forEach(ej => {
                    ...
                    if (imgVal && esSelloVisibleEnRed(ej)) {
                        todasLasPubs.push({
                            id: ej.id,
        ...
        function confirmarAwakePickHoraLibre() {
            ...
            if (!norm) {
                mostrarToastLujo('Escribe una hora válida (HH:MM).');
                if (inp) inp.focus();
                return;
            }
        ...
        async function ejecutarAccionAuth() {
            ...
            if (authMode === 'recover') {
                ...
                try {
                    ...
                    mostrarErrorAuth('');
                    mostrarToastLujo('Si ese correo está en AWAKE, te llegará un enlace');
            ...
            try {
                if (esIdDeseoLocal(d.id)) {
                    ...
                    if (!error && data) d.id = data.id;
                        ...
                        mostrarToastLujo('No se pudo guardar el deseo en la nube. Revisa la conexión.');
                    }
                    return;
                }
        ...
        function aplicarPrefsVisualesRemotas(prefs) {
            ...
            if (prefs.bg_choice != null) {
                ...
                if (fondo !== activeBackground) {
                    ...
                    try { localStorage.setItem('monolith_bg_choice', String(fondo)); } catch (e) {}
                    initActiveBackgroundEngine();
                }
            }
        }
        ...
        function crearTiraSemanaCalendario(weekStart) {
            ...
            for (let i = 0; i < 7; i++) {
                ...
                dayDiv.onclick = (e) => {
                    e.stopPropagation();
                    if (calStripDidSwipe || calStripAnimando) return;
        ...
        function sincronizarPanelAjustes() {
            const s = document.getElementById('switch-sonido');
            const v = document.getElementById('switch-vibracion');
            const ios = document.getElementById('switch-ios-adapt');
            if (s) s.setAttribute('aria-checked', prefsFeedback.sonido ? 'true' : 'false');
        ...
        function alternarPrefFeedback(clave) {
            ...
            sincronizarPanelAjustes();
            if (clave === 'sonido' && prefsFeedback.sonido) {
                const ctx = obtenerAudioCosmico();
                if (ctx) tonoSuaveAwake(ctx, 528, ctx.currentTime, 0.12, 0.13);
            }
        ...
        function setMenuHamburguesaAbierto(abierto) {
            ...
            if (settings) {
                ...
                if (abierto) sincronizarPanelAjustes();
                else cerrarAwakePick();
            }
            if (btn) {
        ...
        function aplicarTemaGlobalHabitos(hue) {
            ...
            styleEl.textContent = `
                ...
                .add-item-btn.is-first {
                    border-color: hsl(${hue}, 80%, 60%) !important;
                    box-shadow: inset 0 1px 0 hsla(${hue}, 90%, 70%, 0.14), 0 0 6px hsla(${hue}, 100%, 55%, 0.12) !important;
                }
                ...
                .badge-nav-btn {
                    border-color: hsl(${hue}, 55%, 42%) !important;
                    color: hsl(${hue}, 82%, 76%) !important;
                    background: linear-gradient(135deg, hsl(${hue}, 55%, 28%) 0%, hsl(${hue}, 70%, 16%) 100%) !important;
                    box-shadow: 0 12px 28px rgba(0, 0, 0, 0.42), inset 0 1px 0 hsla(${hue}, 90%, 70%, 0.12) !important;
                }
                .badge-nav-btn:hover {
                    background: linear-gradient(135deg, hsl(${hue}, 58%, 32%) 0%, hsl(${hue}, 74%, 20%) 100%) !important;
                    border-color: hsl(${hue}, 80%, 65%) !important;
                    box-shadow: 0 12px 28px rgba(0, 0, 0, 0.5), 0 0 14px hsla(${hue}, 100%, 55%, 0.22), inset 0 1px 0 hsla(${hue}, 90%, 80%, 0.2) !important;
                }
            `;
            actualizarMuestraTemaAjustes();
        }
        ...
        async function guardarPerfilModal() {
            ...
            if (currentAvatarSrc && currentAvatarSrc.indexOf('data:') === 0) {
                ...
                if (subida && !esDataUrlMedia(subida)) {
                    ...
                    mostrarToastLujo('No se pudo guardar la foto de perfil.');
                    omitAvatar = true;
                }
            }
        ...
        async function cambiarSocialTab(tipo) {
            ...
            if(btnSeg) btnSeg.classList.toggle('active', tipo === 'seguidores');
            if(btnSegu) btnSegu.classList.toggle('active', tipo === 'seguidos');
        ...
        function renderizarPerfilPublicacionesGrid() {
            ...
            Object.values(dataHistorial).forEach(ejecuciones => {
                ejecuciones.forEach(ej => {
                    const cleanHName = cleanHabitName(ej.nombre);
                    const esVisible = viewingUserId ? esSelloVisibleEnRed(ej) : true;
                    const imgVal = srcImagenHabito((ej.imagenes && ej.imagenes[0]) || ej.image_url);
                    if (esVisible && imgVal) {
        ...
        async function pedirPermisoNotificacionesRitual() {
            ...
            mostrarToastLujo('Permiso de notificaciones pedido');
        }
        ...
        async function abrirAjustesBateriaRitual() {
            ...
            mostrarToastLujo('Ajustes → Aplicaciones → AWAKE → Batería → sin restricciones');
        }
        ...
        const BADGES_UNLOCK_KEY = 'awake_badges_unlocked';
        ...
        function htmlAccionesModalInsignia(item, opts) {
            ...
            const modo = opts.modo || badgeVistaActiva;
            if (modo === 'showcase' || enShow) {
        ...
        function cargarModuloInsignias3D() {
            if (!promesaCargaVisor3D) {
                promesaCargaVisor3D = import('./js/badge-coin-3d.js').catch((err) => {
        ...
        function abrirModalInsignia3D(id) {
            ...
            badgeDetalleId = id;
            const seccionBadges = BADGE_DEFS.filter(d => d.theme === item.theme);
            badgeNavIds = seccionBadges.map(d => d.id);
            ...
            const size = Math.max(240, Math.min(680, window.innerWidth - 48, window.innerHeight - 320));
            const velo = document.getElementById('badge-detail-velo');
            if (velo) velo.classList.add('is-loading');

            (async function iniciarMoneda3DModal() {
                try {
                ...
                if (typeof window.BadgeCoin3D !== 'function') {
                    console.warn('BadgeCoin3D no cargó — revisa consola (módulos ES / servidor local).');
                    if (velo) velo.classList.remove('is-loading');
                    return;
                }
                try {
                    moneda3DModal = new window.BadgeCoin3D(canvas, {
                        size,
                        metalId: metal,
                        themeId: themeIdx >= 0 ? themeIdx : 0,
                        level: item.level,
                        encendida: true,
                        unlockedAt: item.unlockedAt || null,
                        onReady: function () {
                            const v2 = document.getElementById('badge-detail-velo');
                            if (v2) v2.classList.remove('is-loading');
        ...
        function toggleDetalleInsignia(id) {
            ...
            badgeDetalleId = id;
            try { renderizarInsignias(); } catch (e) { console.error('renderizarInsignias (abrir insignia):', e); }
            try { abrirModalInsignia3D(id); } catch (e) { console.error('abrirModalInsignia3D:', e); }
        }
...
try { if (window.awakeAnalytics) window.awakeAnalytics.track('badge_unlocked', { id: def.id }); } catch (e) {}
                    ...
                    revocadas.push(def);
                    changed = true;
                    if (badgeJustUnlockedIds[def.id]) delete badgeJustUnlockedIds[def.id];
                    if (badgeDetalleId === def.id) badgeDetalleId = null;
                }
            });

            if (revocadas.length && Array.isArray(saved._showcase)) {
                const revIds = {};
                revocadas.forEach(d => { revIds[d.id] = true; });
                const nextShow = saved._showcase.filter(id => !revIds[id]);
                if (nextShow.length !== saved._showcase.length) {
                    saved._showcase = nextShow;
                    changed = true;
                }
            }
            ...
            const items = BADGE_DEFS.map(def => {
                const prog = progresoInsignia(def, m);
                const unlockedAt = prog.ok ? (saved[def.id] || null) : null;
                return {
                    ...def,
                    ok: prog.ok,
                    progress: prog.value,
        ...
        function catalogoInsignias() {
            ...
            if (nuevas.length) {
                ...
                encolarToastInsignia({ msg, id: primera.id });
                setTimeout(() => {
                    try {
                        badgeDetalleId = primera.id;
                        renderizarInsignias();
                        abrirModalInsignia3D(primera.id);
                    } catch (e3) {}
                }, 520);
            }
            return items;
            ...
            BADGE_THEMES.forEach(theme => {
                ...
                html += `<div class="badge-theme-block" data-theme="${escapeHtmlChat(theme.id)}">
                    <div class="badge-theme-label">
                        <span class="badge-theme-dot" aria-hidden="true"></span>
        ...
        async function enviarComentarioEnFeed(pubId) {
            ...
            if (typeof reg.id === 'string' && reg.id.startsWith('reg_')) {
                ...
                mostrarToastLujo('Publicación aún no sincronizada. Inténtalo en unos segundos.');
                return;
            }
