/* AWAKE — hábitos, agenda y ficha. Script global; se carga antes de app.js. */
        ...
        function guardarEdicionCatalogoPersonal(habitCleanName, glyph, bgColor) {
            ...
            renderizarBloquesHabitos();
            mostrarToastLujo('Lista actualizada');
            return true;
        }
        ...
        function eliminarDelCatalogoPersonal(nombre, naturaleza) {
            ...
            guardarEstadoLocal();
            renderizarBloquesHabitos();
            mostrarToastLujo('Eliminado de la lista');
        }
        ...
        function guardarDeseoConfigurado() {
            ...
            if (!nombre) {
                mostrarToastLujo('Escribe un nombre.');
                if (el) el.focus();
                return;
            }
        ...
        async function seleccionarDeseoUnico(nombre) {
            ...
            if ((misDeseos || []).some(d => !d.completado && cleanHabitName(d.nombre) === limpio)) {
                ...
                mostrarToastLujo('Ese deseo ya está en tu lista.');
        ...
        function avisarHabitoDuplicado(index) {
            ...
            if (h && habitEsArchivado(h)) {
                mostrarToastLujo(esUna
                    ...
                    : 'Ya existe un hábito como este. Está archivado.');
                return;
            }
            mostrarToastLujo(esUna
                ? 'Ya existe una actividad como esta.'
                : 'Ya existe un hábito como este.');
        }
        ...
        async function guardarHabitoConfigurado() {
            ...
            if (esUnaVez) {
                ...
                if (!fechas.length || !hora) {
                    mostrarToastLujo('Elige al menos un día y la hora.');
                    return;
                }
            ...
            if (!esUnaVez && !tempSelectedHabit.permite247 && necesitaSelectorBloque(tempSelectedHabit) && !configBloqueId) {
                mostrarToastLujo('Elige en qué bloque aparecerá.');
                return;
            }
            if (!esUnaVez && (selectedMomentsConfig.length === 0 || (!tempSelectedHabit.permite247 && !usaN && selectedDaysConfig.length === 0))) {
                mostrarToastLujo(selectedMomentsConfig.length === 0 ? 'Elige al menos un momento del día.' : 'Elige los días o las veces por semana.');
                return;
            }
            ...
            if (esUnaVez) {
                ...
                if (reminderActive) {
                    ...
                    if (!startTime) {
                        mostrarToastLujo('Elige la hora del aviso.');
                        return;
                    }
                } else {
            ...
            if (currentUser) {
                try {
                    ...
                    if (error) {
                        ...
                        mostrarToastLujo('No se pudo guardar el sello.');
                        return null;
                    }
...
try { if (typeof cancelarAvisoRiesgoHoySiProcede === 'function') cancelarAvisoRiesgoHoySiProcede(); } catch (e) {}
        ...
        async function omitirHabitoHoy(index, momento) {
            ...
            if (naturalezaHabito(h) === 'unaVez') {
                mostrarToastLujo('Las actividades de una vez no se omiten. Elimínala o edítala.');
                return;
            }
            if (esHabitoContinuo(h, momento)) {
                mostrarToastLujo('Las abstinencias 24/7 no se omiten.');
                return;
            }
            if (estaCompletadoEnFecha(h, selectedDate, momento)) {
                mostrarToastLujo('Este sello ya está cerrado.');
                return;
            }
        ...
        async function deshacerOmisionHabito(index, momento) {
            ...
            actualizarEstadisticasPerfil();
            try { renderizarInsignias(); } catch (e) {}
            mostrarToastLujo('Omisión deshecha');
        }
        ...
        function solicitarDiaLibre() {
            if (esFechaFutura(selectedDate)) {
                mostrarToastLujo('No se puede marcar un día futuro.');
                return;
            }
            ...
            if (!pendientes.length) {
                mostrarToastLujo('No queda nada discreto por omitir.');
                return;
            }
        ...
        async function aplicarDiaLibre() {
            ...
            mostrarToastLujo('Día libre. La racha sigue.');
        }
        ...
        async function archivarHabito(index) {
            ...
            mostrarToastLujo(`${label} archivad${gen}. Lo encuentras en Historial.`);
        }
        ...
        async function reactivarHabito(index) {
            ...
            renderizarTabHistorial();
            programarAlarmasNativasHabitos();
            mostrarToastLujo('De vuelta en la agenda');
        }
        ...
        async function toggleDescansoHabito(index) {
            ...
            renderizarMiRutina();
            programarAlarmasNativasHabitos();
            mostrarToastLujo(h.enDescanso ? 'En descanso' : 'De vuelta al ritual');
        }
        ...
        async function guardarEdicionHabito() {
            ...
            if (esUnaVez) {
                ...
                mostrarToastLujo(editSelectedMoments.length === 0 ? 'Elige al menos un momento del día.' : 'Elige los días o las veces por semana.');
                return;
            }
        ...
        async function eliminarHabitoReal(index) {
            ...
            actualizarEstadisticasPerfil();
            programarAlarmasNativasHabitos();
            mostrarToastLujo(copiaNaturalezaHabito(h).toastEliminado);
        }
        ...
        function clicCheckboxHabito(index, momento) {
            const h = misHabitos[index];
            if (esFechaFutura(selectedDate)) {
                mostrarToastLujo('Las tareas futuras aún no se pueden completar.');
                return;
            }
            ...
            if (habitEsAbstinencia(h)) {
                ...
                if (estado === 'recaida') {
                    borrarLogDelDia(h, dateTarget, '24/7').then(() => {
                        ...
                        actualizarEstadisticasPerfil();
                        try { renderizarInsignias(); } catch (e) {}
                        mostrarToastLujo('Recaída deshecha');
                    });
                    return;
                }
                resetConfirmModalButtons();
        ...
        async function guardarCompletado() {
            ...
            if (existing) {
                ...
                if (hayArchivo) {
                    comprimirImagenSegura(fileInput.files[0], async (base64Result) => {
                        if (!base64Result) {
                            mostrarToastLujo('No se pudo leer la imagen.');
                            return;
                        }
            ...
            const revertirFalloSello = async (imagenUrl) => {
                pendingHabitMotion = null;
                await borrarVariasMediaAwake(imagenUrl ? [imagenUrl] : []);
                mostrarToastLujo('No se pudo sellar. Inténtalo de nuevo.');
                renderizarMiRutina();
            };
