        ...
        async function subirImagenOAvisar(dataUrl, folder, extHint) {
            ...
            mostrarToastLujo(currentUser ? 'No se pudo guardar la foto. Inténtalo de nuevo.' : 'Inicia sesión para guardar la foto.', { tipo: 'error' });
            return '';
        }
...
try { if (window.awakeAnalytics) window.awakeAnalytics.track('onboarding_done'); } catch (e) {}
        ...
        function actualizarBarraRitual() {
            ...
            bar.innerHTML = `
                <div class="ritual-status-copy">
                    <div class="ritual-status-title"><span class="ritual-count">${done}</span> de ${items.length} sellados${skipped ? ` · ${skipped} omitidos` : ''}</div>
