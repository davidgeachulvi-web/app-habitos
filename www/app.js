        ...
        try { prefsMarco = localStorage.getItem(MARCO_KEY) === '1'; } catch (e) {}
...
try { if (window.awakeAnalytics) window.awakeAnalytics.track('auth_completed'); } catch (e) {}
        ...
        function sincronizarPanelAjustes() {
            ...
            const marco = document.getElementById('switch-marco');
            if (marco) marco.setAttribute('aria-checked', prefsMarco ? 'true' : 'false');
            actualizarMuestraTemaAjustes();
        }

        function ajustarBaseMarco() {
        ...
        function aplicarMarco() {
            ...
            if (el) el.classList.toggle('visible', !!prefsMarco);
            const sw = document.getElementById('switch-marco');
            if (sw) sw.setAttribute('aria-checked', prefsMarco ? 'true' : 'false');
            if (prefsMarco) ajustarBaseMarco();
        }

        function alternarMarco() {
            prefsMarco = !prefsMarco;
            try { localStorage.setItem(MARCO_KEY, prefsMarco ? '1' : '0'); } catch (e) {}
            aplicarMarco();
            if (prefsMarco) { try { reproducirSonido('ajustes_activar'); } catch (e) {} pulsoHaptico(16); }
        }
        ...
        function iniciarMarco() {
            aplicarMarco();
            window.addEventListener('resize', function () { if (prefsMarco) ajustarBaseMarco(); });
            window.addEventListener('orientationchange', function () { if (prefsMarco) ajustarBaseMarco(); });
        }
        ...
        function enviarDiagnostico3D(info) {
            ...
            try {
                ...
                const rastro = [];
                try {
                    const raw = localStorage.getItem((window.awakeAnalytics && window.awakeAnalytics.LOCAL_KEY) || 'awake_analytics_log');
                    if (raw) {
        ...
        function pintarOverlayDebug3D() {
            ...
            const lineas = [];
            try {
                const key = (window.awakeAnalytics && window.awakeAnalytics.LOCAL_KEY) || 'awake_analytics_log';
