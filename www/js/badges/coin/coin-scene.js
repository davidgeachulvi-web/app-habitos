...
export function createCoinStudio(renderer, scene, studioOpts) {
    const sOpts = studioOpts || {};
    const STUDIO = Object.assign({}, STUDIO_DEFAULTS);
    // B-35: presupuesto GPU iOS — si el motor decidió no usar sombras (tier
    // degradado) o un mapa menor, la escena lo respeta.
    if (sOpts.shadowsOn === false) STUDIO.shadow = false;
    const shadowMapSize = sOpts.shadowMapSize || 2048;
    let envTexture = null;
    let envMode = 'awake';
    ...
    keyLight.castShadow = STUDIO.shadow !== false;
